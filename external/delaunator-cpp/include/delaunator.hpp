...
inline float
circumradius(const float ax, const float ay, const float bx, const float by, const float cx, const float cy)
{
...
circumcenter(const float ax, const float ay, const float bx, const float by, const float cx, const float cy)
{
    const float dx = bx - ax;
    const float dy = by - ay;
    const float ex = cx - ax;
...
inline bool in_circle(const float ax,
                      const float ay,
                      const float bx,
                      const float by,
                      const float cx,
...
private:
    ...
    std::size_t
         add_triangle(std::size_t i0, std::size_t i1, std::size_t i2, std::size_t a, std::size_t b, std::size_t c);
    void link(std::size_t a, std::size_t b);
};

Delaunator::Delaunator(std::vector<float> const& in_coords)
    : coords(in_coords), triangles(), halfedges(), hull_prev(), hull_next(), hull_tri(), hull_start(), m_hash(),
      m_center_x(), m_center_y(), m_hash_size(), m_edge_stack()
{
    ...
    if (!(min_dist < std::numeric_limits<float>::max())) {
        std::cout << "not triangulation" << std::endl;
        throw std::runtime_error("not triangulation");
    }
    if (2 * i0 + 1 < 0 || 2 * i0 + 1 >= coords.size()) {
        std::cout << "not triangulation" << std::endl;
        throw std::runtime_error("not triangulation");
    }
    ...
    if (2 * i2 + 1 < 0 || 2 * i2 + 1 >= coords.size()) {
        std::cout << "not triangulation" << std::endl;
        throw std::runtime_error("not triangulation");
    }
...
std::size_t
Delaunator::add_triangle(std::size_t i0, std::size_t i1, std::size_t i2, std::size_t a, std::size_t b, std::size_t c)
{
